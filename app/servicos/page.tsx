...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const description = compactDescription(
    "Conheça os serviços de locação e operações especiais da Eleve para elevação, movimentação e transporte técnico.",
  );
...
export default async function ServicesHubPage() {
  ...
  const categoriesWithCounts = await Promise.all(
    categories.map(async (category) => ({
      ...category,
      services: await listServicesByCategory(category.slug),
    })),
  );
    ...
    <>
      ...
      <main className="min-h-screen bg-white pb-24 pt-36 text-gray-950 dark:bg-[#121212] dark:text-white">
        <section className="mx-auto max-w-6xl px-4 md:px-6">
          ...
          <h1 className="mt-4 text-4xl font-black tracking-[-0.04em] md:text-6xl">Serviços</h1>
          <p className="mt-5 max-w-3xl text-base leading-8 text-gray-700 dark:text-gray-300">
            Explore as categorias de serviços da Eleve para locações e operações especiais com foco em segurança,
