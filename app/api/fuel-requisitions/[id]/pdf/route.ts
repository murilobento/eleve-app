...
import { renderFuelRequisitionPdf } from "@/lib/equipment-requisition-pdf";
import {
  getCompany,
...
export async function GET(
  ...
  try {
    ...
    const requisition = await getFuelRequisitionById(id);

    if (!requisition) {
      return NextResponse.json({ error: "Fuel requisition not found." }, { status: 404 });
    }
    ...
    const pdf = await renderFuelRequisitionPdf({
      requisition,
      company,
      supplier,
    });
