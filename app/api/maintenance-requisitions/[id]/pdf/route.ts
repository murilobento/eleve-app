...
import { renderMaintenanceRequisitionPdf } from "@/lib/equipment-requisition-pdf";
import {
  getCompany,
...
export async function GET(
  ...
  try {
    ...
    const requisition = await getMaintenanceRequisitionById(id);

    if (!requisition) {
      return NextResponse.json({ error: "Maintenance requisition not found." }, { status: 404 });
    }
    ...
    const pdf = await renderMaintenanceRequisitionPdf({
      requisition,
      company,
      supplier,
    });
