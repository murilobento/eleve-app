...
import type { CnpjLookupResult } from "@/lib/company-admin";
...
async function lookupCnpj(cnpj: string): Promise<CnpjLookupResult> {
  ...
  return {
    cnpj: normalizeDigits(payload.cnpj ?? cnpj),
...
export async function GET(request: Request) {
  try {
    ...
    const rateLimitResponse = enforceRateLimit({
      key: `lookup:cnpj:company:${permission.session.user.id}`,
    ...
    return NextResponse.json({ cnpj: result });
  } catch (error) {
