...
export default async function EquipmentCategoryPage({ params }: EquipmentCategoryPageProps) {
  ...
  const jsonLd = [
    ...
    buildItemListJsonLd(
      equipment.map((item) => ({ name: item.name, url: getPublicSiteUrl(`/equipamentos/${item.slug}`) })),
    ...
    <>
      ...
      <main className="min-h-screen bg-white pb-24 pt-36 text-gray-900">
        <section className="mx-auto max-w-6xl px-4 md:px-6">
          <a href="/equipamentos" className="text-xs font-black uppercase tracking-[0.22em] text-[#FCD34D]">Equipamentos</a>
        ...
        <section className="mx-auto mt-14 grid max-w-7xl gap-6 px-4 sm:grid-cols-2 lg:grid-cols-3 md:px-6">
          {equipment.map((item) => (
            ...
            >
              <p className="text-xs font-black uppercase tracking-[0.24em] text-gray-500">{item.capacity}</p>
              <h2 className="mt-3 text-2xl font-black tracking-[-0.03em]">{item.name}</h2>
              <p className="mt-2 text-sm font-semibold uppercase tracking-[0.18em] text-gray-500">{item.model}</p>
              <p className="mt-3 text-sm leading-7 text-gray-500">{item.technicalInfo}</p>
              <a
